...
async function fetchAdventures(city) {
  ...
  try {
    console.log(`url : ${config.backendEndpoint}/adventures?city=${city}`);
...
function addAdventureToDOM(adventures) {
  ...
  adventures.forEach((adventure) => {
    ...
    cardElement.className = "col-6 col-lg-3 mb-3";
    cardElement.innerHTML = `<a id="${adventure.id}" href="detail/?adventure=${adventure.id}"/>
            <div class="card">
            <img class="activity-card" src="${adventure.image}" alt="${adventure.name}">
...
let durations = 0;
if(filters.duration){
  durations = filters.duration.split("-");
}

console.log("durations : " + durations[0]);
let filteredList =[];
